...
(function(b){function e(){var m=document.location,n="ws";"https:"===m.protocol&&(n="wss");return n+("://"+m.host+"/s")}function c(m,n){system.cache.currentService=m;system.cache.currentID=n;system.cache.connectTime=Date.now();g=new WebSocket(e());g.onopen=function(){var p=m;k=!0;l.forEach(a);l=[];p||(p=system.cache.currentService);a({svc:p.key,cmd:command.client.connect,param:system.cache.currentID})};g.onmessage=function(p){p=JSON.parse(p.data);d(p)};g.onerror=function(p){};g.onclose=f}function a(m){k?
(debug&&console.debug("out",m),m=JSON.stringify(m,null,2),g.send(m)):l.push(m)}function d(m){debug&&console.debug("in",m);switch(m.svc){case services.system.key:system.onSystemMessage(m.cmd,m.param);break;case services.team.key:team.onTeamMessage(m.cmd,m.param);break;case services.sprint.key:sprint.onSprintMessage(m.cmd,m.param);break;case services.estimate.key:estimate.onEstimateMessage(m.cmd,m.param);break;case services.standup.key:standup.onStandupMessage(m.cmd,m.param);break;case services.retro.key:retro.onRetroMessage(m.cmd,
m.param);break;default:console.warn("unhandled message for service ["+m.svc+"]")}}function f(){function m(n){debug&&console.info("socket closed, reconnecting in "+n+" seconds");setTimeout(function(){c(system.cache.currentService,system.cache.currentID)},1E3*n)}h||m(10)}var g,k=!1,h=!1,l=[];b.setAppUnloading=function(){h=!0};b.socketConnect=c;b.send=a;b.onSocketMessage=d})(socket||(socket={}));var sprint;
