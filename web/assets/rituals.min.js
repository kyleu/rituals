...
feedback.viewFeedback)};b.open=e;b.openSoon=function(f){setTimeout(function(){return e(f)},0)};b.hide=function(f){f=notify.modal("#modal-"+f);f.$el.classList.contains("uk-open")&&f.hide()}})(modal||(modal={}));var tags;
...
g.key+"?")&&socket.send({svc:g.key,cmd:command.client.removeMember,param:f})};b.saveRole=function(){var f=c();if(f){var g=f.role,k=dom.req("#member-modal-role-select").value;g===k?modal.hide("member"):socket.send({svc:system.cache.currentService.key,cmd:command.client.updateMember,param:{id:f.userID,role:k}})}else console.warn("no active member")};b.applyMembers=function(f){a=f}})(member||(member={}));
