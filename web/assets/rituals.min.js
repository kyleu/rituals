...
(function(c){function e(){console.warn("!!!!!");c.updateSelf(a.filter(c.isSelf).shift());var f=a.filter(function(g){return!c.isSelf(g)});dom.setContent("#member-detail",c.renderMembers(f));0<f.length&&modal.hide("welcome");c.renderOnline()}function b(){if(d){var f=a.filter(function(g){return g.userID===d}).shift();f||console.warn("cannot load active member ["+d+"]");return f}console.warn("no active member")}var a=[],d;c.getMember=function(f){return a.filter(function(g){return g.userID===f}).shift()};
c.getMembers=function(){return a};c.setMembers=e;c.onMemberUpdate=function(f){c.isSelf(f)?modal.hide("self"):modal.hide("member");var g=a,k=g.filter(function(h){return h.userID===f.userID}).shift();k=(null===k||void 0===k?void 0:k.name)!==f.name;g=g.filter(function(h){return h.userID!==f.userID});g.length===a.length&&notify.notify(f.name+" has joined",!0);g.push(f);g.sort(function(h,l){return h.name>l.name?1:-1});a=g;e();c.memberUpdateDom(k)};c.onMemberRemove=function(f){var g;f===system.cache.getProfile().userID?
(notify.notify("you have left this "+(null===(g=system.cache.currentService)||void 0===g?void 0:g.key),!0),document.location.href="/"):(modal.hide("member"),g=a.filter(function(k){return k.userID!==f}),g.sort(function(k,h){return k.name>h.name?1:-1}),a=g,e())};c.viewActiveMember=function(f){f&&(d=f);(f=b())&&c.activeMemberDom(f)};c.removeMember=function(f){(f=void 0===f?d:f)||console.warn("cannot load active member ["+d+"]");"self"===f&&(f=system.cache.getProfile().userID);var g=system.cache.currentService;
confirm("Are you sure you wish to leave this "+g.key+"?")&&socket.send({svc:g.key,cmd:command.client.removeMember,param:f})};c.saveRole=function(){var f=b();if(f){var g=f.role,k=dom.req("#member-modal-role-select").value;g===k?modal.hide("member"):socket.send({svc:system.cache.currentService.key,cmd:command.client.updateMember,param:{id:f.userID,role:k}})}else console.warn("no active member")};c.applyMembers=function(f){a=f};c.loadQR=function(){var f=dom.req("#qrcode");f.src=f.dataset.src}})(member||
(member={}));
...
c.beginEditStory=function(){var a=e(),d=prompt("Edit your story",a.title);d&&d!==a.title&&socket.send({svc:services.estimate.key,cmd:command.client.updateStory,param:{storyID:a.id,title:d}});return!1};c.onRemoveStory=function(){var a=estimate.cache.activeStory;a&&notify.confirm("Delete this story?",function(){socket.send({svc:services.estimate.key,cmd:command.client.removeStory,param:a});modal.hide("story")});return!1};c.getActiveStory=e;c.viewActiveStory=function(a){a&&(estimate.cache.activeStory=
