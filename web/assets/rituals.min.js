...
(function(a){function d(b,a){var c=[];(a?a.querySelectorAll(b):document.querySelectorAll(b)).forEach(function(b){c.push(b)});return c}function c(b,a){a=d(b,a);switch(a.length){case 0:break;case 1:return a[0];default:console.warn("found ["+a.length+"] elements with selector ["+b+"], wanted zero or one")}}function b(b,a){(a=c(b,a))||console.warn("no element found for selector ["+b+"]");return a}function e(a,c){"string"===typeof a&&(a=b(a));a.innerHTML=c;return a}a.initDom=function(b,a){try{style.themeLinks(a),
style.setTheme(b)}catch(h){console.warn("error setting style",h)}try{modal.wire()}catch(h){console.warn("error wiring modals",h)}try{drop.wire()}catch(h){console.warn("error wiring drops",h)}try{tags.wire()}catch(h){console.warn("error wiring tag editors",h)}};a.els=d;a.opt=c;a.req=b;a.setHTML=e;a.setDisplay=function(a,c,e){e=void 0===e?"block":e;"string"===typeof a&&(a=b(a));a.style.display=c?e:"none";return a};a.setContent=function(c,e){"string"===typeof c&&(c=b(c));a.clear(c);c.appendChild(e);
return c};a.setText=function(a,c){"string"===typeof a&&(a=b(a));a.innerText=c;return a};a.clear=function(b){return e(b,"")}})(dom||(dom={}));
...
(function(a){function d(a){if(a.target){var b=a.target;a=b.dataset.key||"";b=b.dataset.t||"";var c=events.getOpenEvent(a);c?c(b):console.warn("no drop open handler registered for ["+a+"]")}}function c(a){if(a.target){var b=a.target;b.classList.contains("uk-open")&&(a=b.dataset.t||"",(b=events.getCloseEvent(b.dataset.key||""))&&b(a))}}function b(a){e&&a.preventDefault()}a.wire=function(){dom.els(".drop").forEach(function(a){a.addEventListener("show",d);a.addEventListener("beforehide",b);a.addEventListener("hide",
c)});events.register("comment",comment.load,comment.closeDrop);events.register("export")};var e=!1;a.onEmojiPicked=function(){e=!0;setTimeout(function(){return e=!1},200)}})(drop||(drop={}));var events;(function(a){var d=new Map,c=new Map;a.register=function(a,e,f){e||(e=function(){});d.set(a,e);f&&c.set(a,f)};a.getOpenEvent=function(a){return d.get(a)};a.getCloseEvent=function(a){return c.get(a)}})(events||(events={}));var modal;
(function(a){function d(a,b){e=b;notify.modal("#modal-"+a).show();return!1}function c(a){if(a.target&&(a=a.target,0===a.id.indexOf("modal"))){a=a.id.substr(6);var b=events.getOpenEvent(a);b?b(e):console.warn("no modal open handler registered for ["+a+"]");e=void 0}}function b(a){a.target&&(a=a.target,a.classList.contains("uk-open")&&(a=a.id.substr(6),(a=events.getCloseEvent(a))&&a(e),e=void 0))}var e;a.wire=function(){dom.els(".modal").forEach(function(a){a.addEventListener("show",c);a.addEventListener("hide",
b)});events.register("welcome");events.register("session",session.onModalOpen);events.register("action",action.loadActions);events.register("self",member.viewSelf);events.register("invitation");events.register("member",member.viewActiveMember);events.register("add-story",story.viewAddStory);events.register("story",story.viewActiveStory);events.register("add-report",report.viewAddReport);events.register("report",report.viewReport);events.register("add-feedback",feedback.viewAddFeedback);events.register("feedback",
feedback.viewFeedback)};a.open=d;a.openSoon=function(a){setTimeout(function(){return d(a)},0)};a.hide=function(a){a=notify.modal("#modal-"+a);a.$el.classList.contains("uk-open")&&a.hide()}})(modal||(modal={}));var tags;
...
b+"]");a=dom.els(".item",a).map(function(a){return a.innerText});dom.setValue("#model-"+b+"-input",a.join(","))}a.wire=function(){dom.els(".tag-editor").forEach(function(a){a.addEventListener("moved",c);a.addEventListener("added",c);a.addEventListener("removed",c)});events.register("choices");events.register("categories")};a.removeTag=function(a){a=a.parentElement;var c=a.parentElement;a.remove();b(c)};a.addTag=function(b){if(b=b.parentElement){var c=a.renderItem();b.insertBefore(c,dom.req(".add-item",
b));d(c)}};a.editTag=d})(tags||(tags={}));
...
content:c}});return!1};a.onRemoveFeedback=function(){var a=retro.cache.activeFeedback;a&&notify.confirm("Delete this feedback?",function(){socket.send({svc:services.retro.key,cmd:command.client.removeFeedback,param:a});modal.hide("feedback")});return!1};a.getActiveFeedback=d;a.viewActiveFeedback=function(a){var b,c=system.cache.getProfile();a&&(retro.cache.activeFeedback=a);(a=d())?(c=a.userID===c.userID,dom.setText("#feedback-title",a.category+" / "+(null===(b=member.getMember(a.userID))||void 0===
b?void 0:b.name)),dom.setSelectOption("#feedback-edit-category",c?a.category:void 0),contents.onContentDisplay("feedback",c,a.content,a.html),comment.setActive("feedback",a.id),comment.setCounts()):console.warn("no active feedback")};a.onFeedbackUpdate=function(a){var e=c(a.id);e.push(a);b(e,a.id)};a.onFeedbackRemoved=function(a){var e=c(a);b(e,a);notify.notify("feedback has been deleted",!0)};a.getFeedbackCategories=function(a,b){var c=b.map(function(b){var c=a.filter(function(a){return a.category===
...
dom.req("#report-edit-date").value,b=dom.req("#report-edit-content").value;socket.send({svc:services.standup.key,cmd:command.client.updateReport,param:{id:standup.cache.activeReport,d:a,content:b}});return!1};a.onRemoveReport=function(){var a=standup.cache.activeReport;a&&notify.confirm("Delete this report?",function(){socket.send({svc:services.standup.key,cmd:command.client.removeReport,param:a});modal.hide("report")});return!1};a.viewActiveReport=function(a){var b,c=system.cache.getProfile();a&&
(standup.cache.activeReport=a);(a=d())?(dom.setText("#report-title",date.toDateString(date.dateFromYMD(a.d))+" - "+(null===(b=member.getMember(a.userID))||void 0===b?void 0:b.name)),b=a.userID===c.userID,dom.setValue(dom.req("#report-edit-date"),b?a.d:""),contents.onContentDisplay("report",b,a.content,a.html),comment.setActive("report",a.id),comment.setCounts()):console.warn("no active report")};a.setReports=function(c){standup.cache.reports=c;dom.setContent("#report-detail",a.renderReports(c));comment.setCounts();
modal.hide("add-report")};a.getReportDates=function(a){return a.map(function(a){return a.d}).filter(function(a,c,d){return d.indexOf(a)===c}).sort().reverse().map(function(b){var c=a.filter(function(a){return a.d===b}).sort(function(a,b){return a.created>b.created?-1:1});return{d:b,reports:c}})};a.viewAddReport=function(){dom.setValue("#report-date",date.dateToYMD(new Date));var a=dom.setValue("#report-content","");dom.wireTextarea(a);setTimeout(function(){return a.focus()},250)};a.viewReport=function(a){report.viewActiveReport(a);
...
(function(a){function d(){var a=document.location,b="ws";"https:"===a.protocol&&(b="wss");return b+("://"+a.host+"/s")}function c(a,c){system.cache.currentService=a;system.cache.currentID=c;system.cache.connectTime=Date.now();g=new WebSocket(d());g.onopen=function(){b({svc:a.key,cmd:command.client.connect,param:c})};g.onmessage=function(a){a=JSON.parse(a.data);e(a)};g.onerror=function(a){};g.onclose=function(){f()}}function b(a){debug&&console.debug("out",a);g.send(JSON.stringify(a))}function e(a){debug&&
console.debug("in",a);switch(a.svc){case services.system.key:system.onSystemMessage(a.cmd,a.param);break;case services.team.key:team.onTeamMessage(a.cmd,a.param);break;case services.sprint.key:sprint.onSprintMessage(a.cmd,a.param);break;case services.estimate.key:estimate.onEstimateMessage(a.cmd,a.param);break;case services.standup.key:standup.onStandupMessage(a.cmd,a.param);break;case services.retro.key:retro.onRetroMessage(a.cmd,a.param);break;default:console.warn("unhandled message for service ["+
...
a.beginEditStory=function(){var a=d(),c=prompt("Edit your story",a.title);if(null===c)return!1;c&&c!==a.title&&socket.send({svc:services.estimate.key,cmd:command.client.updateStory,param:{storyID:a.id,title:c}});return!1};a.onRemoveStory=function(){var a=estimate.cache.activeStory;a&&notify.confirm("Delete this story?",function(){socket.send({svc:services.estimate.key,cmd:command.client.removeStory,param:a});modal.hide("story")});return!1};a.getActiveStory=d;a.viewActiveStory=function(b){b&&(estimate.cache.activeStory=
b);if(b=d())dom.setText("#story-title",b.title),a.viewStoryStatus(b.status),comment.setActive("story",b.id),comment.setCounts()};a.showTotalIfNeeded=c})(story||(story={}));
