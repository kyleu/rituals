namespace story {
  function renderStory(story: story.Story) {
    ...
    return <li id={`story-${story.id}`} class="section" onclick={`modal.open('story', '${story.id}');`}>
      ...
      <div class="left" style="margin-top: 4px;">{comment.renderCount("story", story.id)}</div>
    </li>;
  }
  ...
  export function renderStories(stories: ReadonlyArray<story.Story>) {
    if (stories.length === 0) {
      ...
      return <table class="uk-table uk-table-divider uk-table-small">
  ...
  export function renderTotal(sum: number) {
    return <li id="story-total"><div class="right uk-article-meta"><span class="vote-badge">{sum}</span></div> Total</li>;
  }
