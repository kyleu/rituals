namespace report {
  function renderReport(model: Report) {
    ...
    const ret = <div id={`report-${model.id}`} class="report-detail section" onclick={`modal.open('report', '${model.id}');`}>
  ...
  export function renderReports(reports: ReadonlyArray<Report>) {
    if (reports.length === 0) {
      return <div>
        <button class="uk-button uk-button-default" onclick="modal.open('add-report');" type="button">Add Report</button>
      </div>;
    } else {
