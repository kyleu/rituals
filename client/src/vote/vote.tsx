namespace vote {
  function renderVoteMember(member: member.Member, hasVote: boolean) {
    return (
      <div class="vote-member" title={`${member.name} has ${hasVote ? "voted" : "not voted"}`}>
        <div>
  ...
  function renderVoteResult(member: member.Member, choice: string | undefined) {
    if (!choice) {
      return (
        <div class="vote-result">
          <div>
            <span class="uk-border-circle">
              <span data-uk-icon="icon: minus; ratio: 1.6" />
            </span>
          </div>{" "}
          {member.name}
        </div>
    ...
    return (
      <div class="vote-result">
        <div>
          <span class="uk-border-circle">{choice}</span>
        </div>{" "}
        {member.name}
      </div>
    );
  ...
  export function renderVoteSummary(votes: readonly Vote[]) {
    ...
    return (
      <div class="uk-flex uk-flex-wrap uk-flex-center result-container">
        <div class="result">
          <div class="secondary uk-border-circle">
            {trim(results.count)} / {trim(votes.length)}
          </div>{" "}
          <div>votes counted</div>
        </div>
        <div class="result">
          <div class="secondary uk-border-circle">
            {trim(results.min)}-{trim(results.max)}
          </div>{" "}
          <div>vote range</div>
        </div>
        <div class="result mean-result">
          <div class={`mean uk-border-circle ${system.cache.getProfile().linkColor}-border`}>{trim(results.mean)}</div> <div>average</div>
        </div>
        <div class="result">
          <div class="secondary uk-border-circle">{trim(results.median)}</div> <div>median</div>
        </div>
        <div class="result">
          <div class="secondary uk-border-circle">{trim(results.mode)}</div> <div>mode</div>
        </div>
      </div>
    );
  }
