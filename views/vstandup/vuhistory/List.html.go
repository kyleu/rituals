// Code generated by qtc from "List.html". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line views/vstandup/vuhistory/List.html:1
package vuhistory

//line views/vstandup/vuhistory/List.html:1
import (
	"github.com/kyleu/rituals/app"
	"github.com/kyleu/rituals/app/controller/cutil"
	"github.com/kyleu/rituals/app/lib/filter"
	"github.com/kyleu/rituals/app/standup"
	"github.com/kyleu/rituals/app/standup/uhistory"
	"github.com/kyleu/rituals/views/components"
	"github.com/kyleu/rituals/views/layout"
)

//line views/vstandup/vuhistory/List.html:11
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line views/vstandup/vuhistory/List.html:11
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line views/vstandup/vuhistory/List.html:11
type List struct {
	layout.Basic
	Models              uhistory.StandupHistories
	StandupsByStandupID standup.Standups
	Params              filter.ParamSet
	Paths               []string
}

//line views/vstandup/vuhistory/List.html:19
func (p *List) StreamBody(qw422016 *qt422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vstandup/vuhistory/List.html:19
	qw422016.N().S(`
  <div class="card">
    <div class="right mrs large-buttons">
`)
//line views/vstandup/vuhistory/List.html:22
	if len(p.Models) > 1 {
//line views/vstandup/vuhistory/List.html:22
		qw422016.N().S(`<a href="`)
//line views/vstandup/vuhistory/List.html:22
		qw422016.E().S(uhistory.Route(p.Paths...))
//line views/vstandup/vuhistory/List.html:22
		qw422016.N().S(`/_random"><button>`)
//line views/vstandup/vuhistory/List.html:22
		components.StreamSVGButton(qw422016, "gift", ps)
//line views/vstandup/vuhistory/List.html:22
		qw422016.N().S(` Random</button></a>`)
//line views/vstandup/vuhistory/List.html:22
	}
//line views/vstandup/vuhistory/List.html:22
	qw422016.N().S(`      <a href="`)
//line views/vstandup/vuhistory/List.html:23
	qw422016.E().S(uhistory.Route(p.Paths...))
//line views/vstandup/vuhistory/List.html:23
	qw422016.N().S(`/_new"><button>`)
//line views/vstandup/vuhistory/List.html:23
	components.StreamSVGButton(qw422016, "plus", ps)
//line views/vstandup/vuhistory/List.html:23
	qw422016.N().S(` New</button></a>
    </div>
    <h3>`)
//line views/vstandup/vuhistory/List.html:25
	components.StreamSVGIcon(qw422016, `history`, ps)
//line views/vstandup/vuhistory/List.html:25
	qw422016.N().S(` `)
//line views/vstandup/vuhistory/List.html:25
	qw422016.E().S(ps.Title)
//line views/vstandup/vuhistory/List.html:25
	qw422016.N().S(`</h3>
`)
//line views/vstandup/vuhistory/List.html:26
	if len(p.Models) == 0 {
//line views/vstandup/vuhistory/List.html:26
		qw422016.N().S(`    <div class="mt"><em>No histories available</em></div>
`)
//line views/vstandup/vuhistory/List.html:28
	} else {
//line views/vstandup/vuhistory/List.html:28
		qw422016.N().S(`    <div class="mt">
      `)
//line views/vstandup/vuhistory/List.html:30
		StreamTable(qw422016, p.Models, p.StandupsByStandupID, p.Params, as, ps, p.Paths...)
//line views/vstandup/vuhistory/List.html:30
		qw422016.N().S(`
    </div>
`)
//line views/vstandup/vuhistory/List.html:32
	}
//line views/vstandup/vuhistory/List.html:32
	qw422016.N().S(`  </div>
`)
//line views/vstandup/vuhistory/List.html:34
}

//line views/vstandup/vuhistory/List.html:34
func (p *List) WriteBody(qq422016 qtio422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vstandup/vuhistory/List.html:34
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/vstandup/vuhistory/List.html:34
	p.StreamBody(qw422016, as, ps)
//line views/vstandup/vuhistory/List.html:34
	qt422016.ReleaseWriter(qw422016)
//line views/vstandup/vuhistory/List.html:34
}

//line views/vstandup/vuhistory/List.html:34
func (p *List) Body(as *app.State, ps *cutil.PageState) string {
//line views/vstandup/vuhistory/List.html:34
	qb422016 := qt422016.AcquireByteBuffer()
//line views/vstandup/vuhistory/List.html:34
	p.WriteBody(qb422016, as, ps)
//line views/vstandup/vuhistory/List.html:34
	qs422016 := string(qb422016.B)
//line views/vstandup/vuhistory/List.html:34
	qt422016.ReleaseByteBuffer(qb422016)
//line views/vstandup/vuhistory/List.html:34
	return qs422016
//line views/vstandup/vuhistory/List.html:34
}
