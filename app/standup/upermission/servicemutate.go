...
//nolint:lll
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*StandupPermission) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *StandupPermission, logger util.Logger) error {
	...
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"standup_id\" = $6 and \"key\" = $7 and \"value\" = $8", s.db.Type)
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*StandupPermission) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"standup_id", "key", "value"}, columnsQuoted, s.db.Type)
...
//nolint:lll
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*StandupPermission) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
