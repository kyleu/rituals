...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*StandupHistory) error {
	if len(models) == 0 {
		return nil
	}
...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*StandupHistory) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *StandupHistory, logger util.Logger) error {
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*StandupHistory) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *StandupHistory, _ int) {
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*StandupHistory) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
