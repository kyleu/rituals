...
func (s *Service) GetBySlug(ctx context.Context, tx *sqlx.Tx, slug string, params *filter.Params, logger util.Logger) (Standups, error) {
	params = filters(params)
	wc := "\"slug\" = $1"
	q := database.SQLSelect(columnsString, tableQuoted, wc, params.OrderByString(), params.Limit, params.Offset)
	ret := dtos{}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "unable to get standups by slug [%v]", slug)
	}
	return ret.ToStandups(), nil
}
...
func (s *Service) ListSQL(ctx context.Context, tx *sqlx.Tx, sql string, logger util.Logger) (Standups, error) {
	ret := dtos{}
