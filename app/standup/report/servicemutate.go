...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Report) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			logger.Infof("saving reports [%d-%d]", idx*chunkSize, ((idx+1)*chunkSize)-1)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Report, logger util.Logger) error {
	...
	model.Updated = util.TimeCurrentP()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"id\" = $9", s.db.Placeholder())
	data := model.ToData()
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Report) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
