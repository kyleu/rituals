...
func (s *Service) UpdateVote(storyID uuid.UUID, userID uuid.UUID, choice string) (*Vote, error) {
	...
	if curr == nil {
		...
		s.actions.Post(util.SvcEstimate, *estimateID, userID, action.ActVoteAdd, actionContent, "")
	...
	s.actions.Post(util.SvcEstimate, *estimateID, userID, action.ActVoteUpdate, actionContent, "")
