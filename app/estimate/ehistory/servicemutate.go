...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*EstimateHistory) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *EstimateHistory, _ int) {
		model.Created = util.TimeCurrent()
	})
	q := database.SQLInsert(tableQuoted, columnsQuoted, len(models), s.db.Placeholder())
	vals := lo.FlatMap(models, func(arg *EstimateHistory, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*EstimateHistory) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *EstimateHistory, logger util.Logger) error {
	...
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"slug\" = $5", s.db.Placeholder())
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*EstimateHistory) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *EstimateHistory, _ int) {
	...
	data := lo.FlatMap(models, func(model *EstimateHistory, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*EstimateHistory) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
