...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Vote) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("creating votes [%d-%d]", idx*chunkSize, count)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Vote, logger util.Logger) error {
	...
	model.Updated = util.TimeCurrentP()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"story_id\" = $6 and \"user_id\" = $7", s.db.Type)
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Vote) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"story_id", "user_id"}, columnsQuoted, s.db.Type)
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Vote) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving votes [%d-%d]", idx*chunkSize, count)
		}
