...
func (s *Service) ListWhere(ctx context.Context, tx *sqlx.Tx, where string, params *filter.Params, logger util.Logger, values ...any) (Stories, error) {
	params = filters(params)
...
func (s *Service) SearchEntries(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (result.Results, error) {
	...
	return lo.Map(ret, func(m *Story, _ int) *result.Result {
		return result.NewResult("Story", m.String(), m.WebPath(), m.TitleString(), "story", m, m, query)
	}), nil
}
