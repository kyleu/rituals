...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Estimate) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Estimate, _ int) {
	...
	vals := lo.FlatMap(models, func(arg *Estimate, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Estimate) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("creating estimates [%d-%d]", idx*chunkSize, count)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Estimate, logger util.Logger) error {
	curr, err := s.Get(ctx, tx, model.ID, logger)
	if err != nil {
		return errors.Wrapf(err, "can't get original estimate [%s]", model.String())
	}
	model.Created = curr.Created
	model.Updated = util.TimeCurrentP()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"id\" = $11", s.db.Type)
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Estimate) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Estimate, _ int) {
	...
	data := lo.FlatMap(models, func(model *Estimate, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Estimate) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving estimates [%d-%d]", idx*chunkSize, count)
		}
