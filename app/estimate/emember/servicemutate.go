...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *EstimateMember, logger util.Logger) error {
	...
	model.Updated = util.NowPointer()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"estimate_id\" = $8 and \"user_id\" = $9", "")
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*EstimateMember) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"estimate_id", "user_id"}, columnsQuoted, "")
	var data []any
	for _, model := range models {
