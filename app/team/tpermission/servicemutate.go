...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *TeamPermission, logger util.Logger) error {
	...
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"team_id\" = $6 and \"key\" = $7 and \"value\" = $8", s.db.Placeholder())
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*TeamPermission) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"team_id", "key", "value"}, columnsQuoted, s.db.Placeholder())
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*TeamPermission) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
