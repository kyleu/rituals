...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *TeamPermission, logger util.Logger) error {
	curr, err := s.Get(ctx, tx, model.TeamID, model.K, model.V, logger)
	if err != nil {
		return errors.Wrapf(err, "can't get original permission [%s]", model.String())
	}
	model.Created = curr.Created
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"team_id\" = $6 and \"k\" = $7 and \"v\" = $8", "")
	data := model.ToData()
	data = append(data, model.TeamID, model.K, model.V)
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*TeamPermission) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"team_id", "k", "v"}, columnsQuoted, "")
	var data []any
	for _, model := range models {
...
func (s *Service) Delete(ctx context.Context, tx *sqlx.Tx, teamID uuid.UUID, k string, v string, logger util.Logger) error {
	q := database.SQLDelete(tableQuoted, defaultWC(0))
	_, err := s.db.Delete(ctx, q, tx, 1, logger, teamID, k, v)
	return err
}
