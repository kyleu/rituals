...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *TeamMember, logger util.Logger) error {
	...
	model.Updated = util.NowPointer()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"team_id\" = $8 and \"user_id\" = $9", "")
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*TeamMember) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"team_id", "user_id"}, columnsQuoted, "")
	var data []any
	for _, model := range models {
