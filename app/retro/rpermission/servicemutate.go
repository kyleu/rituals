...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *RetroPermission, logger util.Logger) error {
	...
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"retro_id\" = $6 and \"key\" = $7 and \"value\" = $8", "")
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*RetroPermission) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"retro_id", "key", "value"}, columnsQuoted, "")
	var data []any
	for _, model := range models {
