...
//nolint:lll
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*RetroPermission) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *RetroPermission, logger util.Logger) error {
	curr, err := s.Get(ctx, tx, model.RetroID, model.Key, model.Value, logger)
	if err != nil {
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*RetroPermission) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
