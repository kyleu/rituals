...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *RetroMember, logger util.Logger) error {
	...
	model.Updated = util.NowPointer()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"retro_id\" = $8 and \"user_id\" = $9", "")
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*RetroMember) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"retro_id", "user_id"}, columnsQuoted, "")
	var data []any
	for _, model := range models {
