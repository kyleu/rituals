...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*RetroMember) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *RetroMember, logger util.Logger) error {
	...
	model.Updated = util.TimeCurrentP()
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"retro_id\" = $8 and \"user_id\" = $9", s.db.Placeholder())
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*RetroMember) error {
	...
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"retro_id", "user_id"}, columnsQuoted, s.db.Placeholder())
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*RetroMember) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
