...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Feedback) error {
	if len(models) == 0 {
		return nil
	}
...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Feedback) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Feedback, logger util.Logger) error {
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Feedback) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Feedback, _ int) {
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Feedback) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving feedbacks [%d-%d]", idx*chunkSize, count)
		}
