...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*RetroHistory) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *RetroHistory, _ int) {
		model.Created = util.TimeCurrent()
	})
	q := database.SQLInsert(tableQuoted, columnsQuoted, len(models), s.db.Placeholder())
	vals := lo.FlatMap(models, func(arg *RetroHistory, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*RetroHistory) error {
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *RetroHistory, logger util.Logger) error {
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*RetroHistory) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *RetroHistory, _ int) {
	...
	data := lo.FlatMap(models, func(model *RetroHistory, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*RetroHistory) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
