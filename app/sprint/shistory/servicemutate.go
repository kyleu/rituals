...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*SprintHistory) error {
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*SprintHistory) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
