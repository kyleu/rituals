...
func check(s *npnconnection.Service, userID uuid.UUID, auths auth.Records, teamID *uuid.UUID, sprintID *uuid.UUID, svc string, modelID uuid.UUID) (permission.Permissions, permission.Errors) {
...
func checkPerms(s *npnconnection.Service, userID uuid.UUID, teamID *uuid.UUID, sprintID *uuid.UUID, svc string, modelID uuid.UUID) error {
	auths := s.Auths.GetByUserID(userID, nil)
	_, permErrors := check(s, userID, auths, teamID, sprintID, svc, modelID)
	if len(permErrors) > 0 {
