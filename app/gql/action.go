...
func initAction() {
	actionResolver = func(p graphql.ResolveParams, ctx web.RequestContext) (interface{}, error) {
		...
		return ctx.App.Action.GetByID(*id)
	}
	...
	actionAuthorResolver = func(p graphql.ResolveParams, ctx web.RequestContext) (interface{}, error) {
		return ctx.App.User.GetByID(p.Source.(*action.Action).AuthorID, false)
	}
