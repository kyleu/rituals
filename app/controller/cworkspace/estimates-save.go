...
func EstimateSave(rc *fasthttp.RequestCtx) {
	controller.Act("workspace.estimate.save", rc, func(as *app.State, ps *cutil.PageState) (string, error) {
		...
		_, msg, u, err := as.Services.Workspace.ActionEstimate(ps.Context, slug, frm, ps.Profile.ID, ps.Logger)
		if err != nil {
			return "", err
		}
